...
public class ScreenPizzaStation extends AbstractContainerScreen<PizzaStationMenu> implements MenuAccess<PizzaStationMenu>, ContainerListener {
    ...
    protected void renderTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
        ...
        if(getGuiLeft() + 116 <= pX && pX <= getGuiLeft() + 22 + 116 && getGuiTop() + 66 <= pY && pY <= getGuiTop() + 15 + 66) {
            List<Component> components = new ArrayList<>();
            components.add(Component.translatable("information.pizzadelight.view_recommended_ingredients"));
            pGuiGraphics.renderTooltip(font, components, Optional.of(new IngredientsTooltip()), pX, pY);
        }
    }
